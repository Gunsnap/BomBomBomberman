...
///     characterized by the <see cref="radius"><c>radius</c></see> (distance
///     from the centre of a hexagon to one of its vertices) and the
///     <see cref="depth"><c>depth</c></see> (distance between two honeycomb
///     layers). Hex grids use a herringbone pattern for their coordinate
...
public class GFHexGrid : GFLayeredGrid {
	...
	public enum HexOrientation {
		/// Pointy east and west, flat north and south, equal to
		/// <see cref="HexTopOrientation.FlatTops"><c>HexTopOrientation.FlatTops</c></see>.
		PointySides,

		/// Flat east and west, pointy north and south, equal to
		/// <see cref="HexTopOrientation.FlatTops"><c>HexTopOrientation.PointyTops</c></see>.
		FlatSides
	};
	...
	/// <remarks>
	///   <param>
	///     There are two ways a hexagon can be rotated: <c>FlatTops</c> has
	///     flat tops, parallel to the grid's X-axis, and <c>PointyTops</c> has
	///     pointy tops, parallel to the grid's Y-axis. This enum is best used
	///     to complement <see cref="HexOrientation"><c>HexOrientation</c></see>.
	///   </param>
	/// </remarks>
	public enum HexTopOrientation {
		/// Flat north and south, pointy east and west, equal to
		/// <see cref="HexOrientation.PointySides"><c>HexOrientation.PointySides</c></see>.
		FlatTops,

		/// Pointy north and south, flat east and west, equal to
		/// <see cref="HexOrientation.FlatSides"><c>HexOrientation.FlatSides</c></see>.
		PointyTops
	};
	...
	private enum HexCoordinateSystem {
		...
		/// is always 0.
		Cubic,
		//Barycentric  ///< Don't use, not yet ready!
	};

	/// <summary>
	///   Shape of the drawing and rendering.
	/// </summary>
	/// <remarks>
	///   <para>
	...
	/// <summary>
	///   Internal radius member, not to be set directly.
	/// </summary>
	[SerializeField] private float _radius = 1.0f;

	/// <summary>
	///   Internal orientation of the hex grid, not to be set directly.
	/// </summary>
	[SerializeField] protected HexOrientation _hexSideMode;

	/// <summary>
	///   Visual shape of the grid to render, do not set directly.
	/// </summary>
	[SerializeField] protected HexGridShape _gridStyle = HexGridShape.Rectangle;
	...
	/// <remarks>
	///   <para>
	///	    This refers to the distance between the centre of a hexagon and one
	///	    of its vertices. Since the hexagon is regular all vertices have the
	///	    same distance from the centre. In other words, imagine a
	///	    circumscribed circle around the hexagon, its radius is the radius
	///	    of the hexagon. The value may not be less than `Mathf.Epsilon`.
	///   </para>
	/// </remarks>
	public float radius {
		get { return _radius; }
	...
	/// <summary>
	///   Hex to render the cone around (grid coordinates).
	/// </summary>
	...
	///
	/// <value>
	///   Whether the grid has pointy sides or flat sides. This affects both
	///   the drawing and the calculations.
	/// </value>
	...
	///   connected to <see cref="hexSideMode"><c>hexSideMode</c></see>, in
	///   fact this is just an accessor that gets and sets the appropriate
	///   value for it.
	/// </value>
	...
	///
	/// <value>
	///   This is the full vertical height of a hex.  For pointy side hexes
	///   this is the distance from one edge to its opposite (<c>sqrt(3) *
	///   radius</c>) and for flat side hexes it is the distance between two
	///   opposite vertices (<c>2 * radius</c>).
	/// </value>
	...
	///
	/// <value>
	///   This is the full horizontal width of a hex.  For pointy side hexes
	///   this is the distance from one vertex to its opposite (<c>2 *
	///   radius</c>) and for flat side hexes it is the distance between to
	///   opposite edges (<c>sqrt(3) * radius</c>).
	/// </value>
